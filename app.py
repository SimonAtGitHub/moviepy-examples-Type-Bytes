...
def create_typing_video(
    ...
    for char in code_text:
        ...
        if format_choice == "Paragraph":
            wrapped_lines = textwrap.wrap(
                current_text, width=max_width // font.getlength(" ")
            )
        else:  # Programming
    ...
    speed_factor = {
        ...
        video_speed, 1.0
    )  # Default to 1.0 if video_speed is not found in the dictionary

    video = VideoFileClip(video_filename).with_effects(
    ...
    if enhance_quality:
        ...
        video = video.with_effects(
            [vfx.Resize(height=720)]
        )  # Resize video to enhance quality
...
format_choice = gr.Dropdown(
    choices=["Paragraph", "Programming"], value="Paragraph", label="Text Format"
)

line_spacing = gr.Dropdown(
    choices=[1.0, 1.15, 1.5, 2.0, 2.5, 3.0], value=1.5, label="Line Spacing"
)
...
font_size = gr.Dropdown(
    choices=["16", "18", "20", "22", "24"], value="18", label="Font Size"
)

width_choice = gr.Dropdown(
    choices=["400", "800", "1024", "1280", "1920"], value="800", label="Video Width"
)

height_choice = gr.Dropdown(
    choices=["400", "720", "1080", "1440", "2160"], value="400", label="Video Height"
)
...
custom_audio = gr.File(label="Upload Custom Audio SFX🔊", type="filepath")
...
enhance_quality = gr.Checkbox(label="Enhance Video Quality")

video_speed = gr.Dropdown(
    choices=["1x", "1.25x", "1.5x", "1.75x", "2x"], value="1x", label="Video Speed"
)
...
iface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(
            label="Enter Content",
            lines=10,
            placeholder="Enter the text to be displayed in the video...",
        ),
        format_choice,
        line_spacing,
        width_choice,
